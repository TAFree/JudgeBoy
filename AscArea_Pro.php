...
class AscArea implements Logic {
    ...
    public function parseMatches($matches, $found, &$response = null){
        ...
        if ($found) { // Matched	
            ...
            for($i=2; $i <= 5; $i++){
                $this->isAsc($matches[$i-1], $matches[$i]);
            }
            
            if(in_array('>', $this->cmpopt)){
                for($i=2; $i <= 5; $i++){
    ...
    private function isAsc($area1, $area2){
        if($area1 < $area2){
	...
	switch($mode) {
		...
		case 'Cmp':
			if ($item=='>') {
				$this->response .=<<<EOF
<tr><td class='ITEM_TD'><p>{$inner[0]}</p></td><td class='ITEM_TD'><p class='WRONG_P'>$item</p></td><td class='ITEM_TD'><p>{$inner[1]}</p></td></tr>
EOF;
			}
			else{
				$this->response .=<<<EOF
<tr><td class='ITEM_TD'><p>{$inner[0]}</p></td><td class='ITEM_TD'><p>$item</p></td><td class='ITEM_TD'><p>{$inner[1]}</p></td></tr>
EOF;
			}
			break;
   	 }
...
class Custom {
	...
	public function __construct () {
		...
		foreach ($this->testdata as $key => $value) {
			...
			$pattern = '/Area:&#9633;(\d+\.\d+).*Area:&#9633;(\d+\.\d+).*Area:&#9633;(\d+\.\d+).*Area:&#9633;(\d+\.\d+).*Area:&#9633;(\d+\.\d+)/';			
			$matcher = new Matcher($pattern, $student_output[1], new AscArea(), $response);
